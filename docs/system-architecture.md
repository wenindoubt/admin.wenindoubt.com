...
- **Supabase** is used only for Realtime subscriptions (Kanban board live sync). DB queries go through Drizzle ORM directly to Postgres. Realtime is authenticated via Clerk third-party JWT — Supabase verifies tokens against Clerk's JWKS endpoint. RLS on `deals` and `notes` restricts anonymous access; `gmail_tokens` has an explicit deny-anon policy.
