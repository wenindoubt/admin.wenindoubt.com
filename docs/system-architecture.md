...
- **Supabase** is used for Realtime subscriptions (Kanban board live sync) and file storage (note attachments via `note-attachments` bucket). DB queries go through Drizzle ORM directly to Postgres. Realtime is authenticated via Clerk third-party JWT — Supabase verifies tokens against Clerk's JWKS endpoint. RLS enabled + forced on all 12 tables with SELECT-only policies for `authenticated`; `anon` fully revoked. `gmail_tokens` has an owner-only RLS policy (JWT `sub` claim must match `clerk_user_id`). Default EXECUTE privilege on functions revoked from `anon`/`public`.
...
- **CI/CD** — GitHub Actions for CI (lint, typecheck, build, unit tests, Semgrep SAST). `migrate.yml` runs `supabase db push` against production on merge to main when `supabase/migrations/**` changes. Vercel deploys the app. Migrations complete in seconds; Vercel build takes 1-3 minutes, so schema is always ready before the new app goes live. Supabase migrations are the schema source of truth; Drizzle push is a local-only sync check.
