...
- **RLS**: enabled + forced on all 12 tables. `authenticated` gets SELECT-only (all writes via server actions as `postgres`). `anon` fully revoked. `gmail_tokens` has an additional explicit deny-anon policy. RLS on `deals` enforced on Supabase Realtime subscriptions.
