...
- **RLS**: enabled + forced on all 12 tables. `authenticated` gets SELECT-only (all writes via server actions as `postgres`). `anon` fully revoked. `gmail_tokens` has an owner-only RLS policy (`clerk_user_id` must match JWT `sub` claim). Default EXECUTE privilege on functions revoked from `anon`/`public`. RLS on `deals` enforced on Supabase Realtime subscriptions.
- **Note attachments**: stored in Supabase Storage (`note-attachments` bucket), metadata in `note_attachments` table. Cascade-deleted with parent note. RLS enabled with SELECT-only for authenticated (file access via server-side admin client).
