...
/** Create a contact via the standalone form and return the firstName */
async function createTestContact(crm: {
...
test.describe("Contacts — CRUD", () => {
  ...
  test("edit contact", async ({ crm }) => {
    const firstName = await createTestContact(crm);

    await crm.goto("/contacts");
    await crm.waitForContentLoad();
    await crm.search("Search contacts...", firstName);
    ...
    const editPath = `${new URL(crm.page.url()).pathname}/edit`;
    await crm.goto(editPath);
    await crm.waitForContentLoad();
