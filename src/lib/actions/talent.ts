...
export async function findTalentForEntity(
  ...
  const entityNoteRows = await db
    .select({ content: notes.content })
    .from(notes)
    .where(
      and(
        entityType === "company"
          ? eq(notes.companyId, entityId)
          : eq(notes.contactId, entityId),
        isNotNull(notes.content),
      ),
    );

  if (entityNoteRows.length === 0) return [];

  const context = entityNoteRows.map((n) => n.content).join("\n\n");
  const queryEmbedding = await generateEmbedding(context, "RETRIEVAL_QUERY");
  const similarity = sql<number>`1 - (${cosineDistance(notes.embedding, queryEmbedding)})`;
